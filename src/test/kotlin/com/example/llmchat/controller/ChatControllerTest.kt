...
class ChatControllerTest {
    ...
    @Test
    fun `POST new chat with empty title returns 400 Bad Request`() {
        mockMvc.perform(post("/chat").param("title", ""))
            .andExpect(status().isBadRequest)
    }

    @Test
    fun `GET chat with invalid ID format handles gracefully`() {
        val invalidChatId = "invalid-id"
        `when`(chatService.getChat(invalidChatId)).thenThrow(IllegalArgumentException("Invalid chat ID format: $invalidChatId"))

        mockMvc.perform(get("/chat/{chatId}", invalidChatId))
            .andExpect(status().isBadRequest)
    }

    @Test
    fun `DELETE chat with invalid ID format handles gracefully`() {
        val invalidChatId = "invalid-id"
        `when`(chatService.deleteChat(invalidChatId)).thenThrow(IllegalArgumentException("Invalid chat ID format: $invalidChatId"))
        
        mockMvc.perform(delete("/chat/{chatId}", invalidChatId))
            .andExpect(status().isBadRequest)
    }
}
